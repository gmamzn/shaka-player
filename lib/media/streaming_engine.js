...
shaka.media.StreamingEngine = class {
  ...
  /**
   * Tries to avoid duplicate segment ref due to inaccuracy
   * (if found).
   * @param {!shaka.media.StreamingEngine.MediaState_} mediaState
  ...
  tryAvoidDuplicateSegmentRef_(mediaState, ref, time, reverse) {
    ...
    if (ref != null && ref != origRef) {
      shaka.log.info(logPrefix, 'tryAvoidDuplicateSegmentRef_',
    ...
    // If we are here, we wanted to find a segment for
    // adjusted time but couldn't. This means we will
    // possibly see AV sync issues, but we can only warn
    // and return original segment ref.
    ...
    if (mediaState.segmentIterator) {
      ...
      if (ref != null &&
          shaka.util.NumberUtils.isFloatEqual(
              ref.endTime, time, 0.15)) {
        ref = this.tryAvoidDuplicateSegmentRef_(
